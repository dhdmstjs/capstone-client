...
export default {
  ...
  methods: {
    route(type) {
      if (type == "home") {
        window.location.replace('http://206.189.79.86:8080/')
      }
      if (type == "roster") {
        window.location.replace('http://206.189.79.86:8080/posts/'+this.class)
      }
      if (type == "attendance") {
        let today = new Date()
        ...
        today = yyyy+"-"+mm+"-"+dd
        window.location.replace('http://206.189.79.86:8080/view/'+this.class+'/'+today)
